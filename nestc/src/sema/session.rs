...
pub struct Session {
    ...
    /// Packages read in from a library's metadata rather than compiled here, in
    /// the order they were read (`crate::library`).
    pub libraries: Vec<crate::library::Loaded>,
    /// The first IR id this session's own lowering allocated: every id below it
    /// belongs to a library.
    pub own_ir_base: u32,
    /// How many IR ids there were when monomorphization started. A library's
    /// metadata carries the facts on ids below this and not the ones
    /// monomorphization's copies are stamped with, which it rebuilds.
    pub ir_before_mono: u32,
...
impl Session {
    ...
    /// reader can look at.
    pub(crate) fn target_module_source(&self) -> String {
        let o = &self.options;
        format!(
    ...
    /// Make `root`, a file read from a library, what `import <name>` loads.
    pub fn adopt_library_root(&mut self, name: &str, root: FileId) {
        self.cache.insert(format!("pkg:{name}"), root);
    }

    /// The library `file` was read in from, if it was.
    pub fn library_of_file(&self, file: FileId) -> Option<&crate::library::Loaded> {
        self.libraries.iter().find(|l| l.owns_file(file))
    }

    /// Whether `file` came from a library, and so was analyzed where the library
    /// was compiled rather than here.
    pub fn is_foreign_file(&self, file: FileId) -> bool {
        self.library_of_file(file).is_some()
    }
